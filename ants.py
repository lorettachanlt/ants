...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if self.ant.name != 'Queen' or self.ant.num_queens > 1:
                self.ant = None
                insect.place = None
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.num_queens = self.counter + 1
        QueenAnt.counter = self.num_queens
    ...
    def action(self, colony):
        ...
        if self.num_queens > 1:
            self.place.ant.reduce_armor(1)
        else:
            ThrowerAnt.action(self, colony)
            queen_place = self.place
            ant_places = queen_place.exit
            while ant_places != None and ant_places.ant != None:
                if ant_places not in self.doubled:
                    ant_places.ant.damage *= 2
                    self.doubled.append(ant_places)
                ant_places = ant_places.exit
        colony_queen = colony.queen
        colony.queen = queen_place
        x = QueenPlace()
        x.add_bees(colony_queen, queen_place)
        colony.queen.bees = x.bees

class QueenPlace(object):
    bees = []
    def add_bees(self, colony, queen_place):
        
        if len(queen_place.bees) > 0 and len(colony.bees) > 0:
            self.bees = queen_place.bees
            for bee in colony.bees[:]:
                self.bees.append(bee)
        elif len(queen_place.bees) > 0:
            self.bees = queen_place.bees
        else:
            self.bees = colony.bees
